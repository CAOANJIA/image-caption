...
class EncoderCNN(nn.Module):
    def __init__(self):
        ...
        res = nn.Sequential(*list(res.children())[: -1])    # 去除fc
        res = nn.Sequential(*list(res.children())[: -1])    # 去除maxpoll
        self.res = res

    def forward(self, images):
        output = self.res(images)   # 输出为[batch_size, 2048, 7, 7]
        output = output.permute(0, 2, 3, 1)
        return output               # [batch_size, 7, 7, 2048]
...
class DecoderLSTM(nn.Module):
    def __init__(self, decoder_dim, att_dim, embed_dim, vocab_size, encoder_dim=2048, dropout=0.5):
