...
1. 优化器、学习率的重要性：学习率不应太大也不应太小，太大很难收敛，太小训练较慢，我在实验中曾尝试了~~Adadelta（lr为1e-3和1e-2）~~和~~Adam（lr为1e-2，1e-3和1e-4）~~，效果均不够理想，最终选择了``Adam``，对于pretrain阶段，将前五个epoch的``lr``设为``5e-4`` ，中间3个epoch设为``(5e-4)/2``，最后2个epoch设为``(5e-4)/4``，finetune阶段则设置初始``lr``为``1e-4``，每个epoch递减20%
